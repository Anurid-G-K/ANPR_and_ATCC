...
def flush_results_csv_and_ui(best_vehicle_log, csv_path, frame_key):
    ...
    if rows:
        ...
        unique_key = f"{frame_key}_{datetime.now().strftime('%H%M%S%f')}"
        ...
        download_button_placeholder.download_button(
            ...
            mime='text/csv',
            key=f"download_results_{unique_key}"
        )
...
while ret:
    ...
    # Resize the frame to 75% for display only
    display_height = int(height * 0.75)
    display_width = int(width * 0.75)
    frame_resized = cv2.resize(frame_rgb, (display_width, display_height))
    
    frame_placeholder.image(frame_resized, channels="RGB", use_container_width=False)
...
flush_results_csv_and_ui(best_vehicle_log, results_csv_path, frame_nmr + 1)
